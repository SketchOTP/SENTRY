# Authority Project-State Index

## Project identity
- Project: SENTRY
- Authority schema: 3.0
- Canonical Notion: https://app.notion.com/p/3c5833cb27ff8065aa88eff1089970a8
- GitHub: https://github.com/SketchOTP/SENTRY

## Current pointers
- Current stage: V0.3.2 bounded conversational turn-taking remains active; V0.3.1 always-available voice foundation is qualified at `a3f6d67`.
- Active directive: `SENTRY-V0.3.2-BOUNDED-TURN-TAKING-001`; the operator's read-only web override is implemented through native Luna/Codex web search.
- Active task packet: add a host-bounded, RAM-only, wake-free follow-up focus session after successful V0.3.1 voice answers. Public research uses the planner-authorized `use_native_web_search` marker and a request-scoped native Luna/Codex web-search synthesis turn; V0.3.2 remains unaccepted.
- Last accepted outcome: the local Vosk → Whisper → bounded conversation orchestrator → Kokoro office interface is qualified, with current physical truthfulness and private NWS weather retained.
- Latest recorded outcome: the former host-transport public-web implementation is superseded by the operator-selected native Luna/Codex read-only web-search path. A real native-web Mount Washington forecast proof and the full Ubuntu regression `232/232` passed; V0.3.2 remains unaccepted.
- Last state sync: 2026-09-01; native Luna web-search correction is implemented at `564ba78`; V0.3.2 remains active from `a3f6d67`.

## Mandatory kernel
Read these before substantial work:
1. `PROJECT_GOAL.md`
2. `PROJECT_PROFILE.md`
3. `CURRENT.md`

Then read the active directive from `DIRECTIVES.md` and retrieve only the relevant entries from the historical ledgers below.

## Historical ledgers
- `DIRECTIVES.md` — issued work and acceptance boundaries.
- `OUTCOMES.md` — what happened and the evidence achieved.
- `LEARNINGS.md` — durable verified technical/project learnings.
- `RECORD.md` — major decisions, milestones, reversals, governance events.
- `REPO_MAP.md` — repository structure and important boundaries.
- `EXTERNAL.md` — relevant external prior art and dispositions.

Do not bulk-load entire growing ledgers unless the task genuinely requires it. Do not skip relevant history merely to save context.

## Update rule
`CURRENT.md` is the mutable current snapshot.
Historical ledgers are append-only after adoption. Correct mistakes with a new superseding entry; do not rewrite old evidence.
