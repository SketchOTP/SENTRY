...
- `person-detection-0303` is the official Open Model Zoo FP32 XML/BIN artifact. Its manifest-provided SHA-384 checksums are recorded in Authority and the files remain outside Git under `perception-data/models/person-detection-0303/FP32/`. The model uses native `1x3x720x1280` BGR input and emits `boxes` `(N,5)` plus a companion `labels` tensor. The reference `class_agnostic_detection` semantics use positive box confidence, ignore companion labels for class selection, scale with `[1/1280, 1/720]`, assign person label `1`, and apply class-agnostic NMS at `0.6`; SENTRY now reproduces that path before its detector contract.
